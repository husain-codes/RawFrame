...
void test_img_save_ppm_null() {
  TEST_ASSERT_EQUAL(-1, img_save_ppm("test_output.ppm", NULL));
}

void test_img_save_ppm_roundtrip() {
  img_t *image = img_create(2, 2, IMG_FMT_RGB24);
  TEST_ASSERT_NOT_NULL(image);
  pixel_t p1 = {255, 0, 0, 255};
  pixel_t p2 = {0, 255, 0, 255};
  pixel_t p3 = {0, 0, 255, 255};
  pixel_t p4 = {255, 255, 0, 255};
  img_set_pixel(image, 0, 0, p1);
  img_set_pixel(image, 1, 0, p2);
  img_set_pixel(image, 0, 1, p3);
  img_set_pixel(image, 1, 1, p4);

  TEST_ASSERT_EQUAL(0, img_save_ppm("test_output.ppm", image));

  img_t *loaded_image = img_load_ppm("test_output.ppm");
  TEST_ASSERT_NOT_NULL(loaded_image);
  TEST_ASSERT_EQUAL(2, loaded_image->width);
  TEST_ASSERT_EQUAL(2, loaded_image->height);
  TEST_ASSERT_EQUAL(IMG_FMT_RGB24, loaded_image->format);
  ...
  TEST_ASSERT_EQUAL_UINT8(0, out.r);
  TEST_ASSERT_EQUAL_UINT8(0, out.g);
  TEST_ASSERT_EQUAL_UINT8(255, out.b);
...
void test_img_save_bmp_bgr_roundtrip() {
  ...
  TEST_ASSERT_EQUAL(6,
                    image->stride[0]); // 2 pixels * 3 bytes per pixel for BGR24
...
int main() {
  ...
  RUN_TEST(test_img_save_ppm_null);
  RUN_TEST(test_img_save_ppm_roundtrip);
